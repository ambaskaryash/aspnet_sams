...
; (function ($) {
    ...
    $.makeSuggest = function (area, suggests, options) {
        ...
        //        var suggest = suggests[i].toLowerCase();
        ...
        //    if (e.keyCode == KEY.TAB || e.keyCode == KEY.UP || e.keyCode == KEY.DOWN) {
        //        if ($area.options.cycleOnTab) {
        //            var chunk = $area.getChunk();
        //            if (chunk.length >= $area.options.minChunkSize) {
        //                var selectedText = $area.getCompletion(performCycle = true);
        //                var firstChar = chunk.substring(0, 1);
        //                firstChar = firstChar.toUpperCase();
        //                var tail = chunk.substring(1);
        //                chunk = firstChar + tail;
        //                var ddlText = (chunk + selectedText);
        //                $("#txtAutoSearch").attr('value', ddlText).attr('selected', true);
        //                $area.updateSelection(selectedText);
        //            }
        //            e.preventDefault();
        //            e.stopPropagation();
        //            $area.focus();
            ...
            switch (e.keyCode) {
                ...
                default:
                    if (!hasSpecialKeys && $area.options.autoComplete) {
                        var chunk = $area.getChunk();
                        var enteredText = $area.val();

                      if (chunk == "") {
                            var textBeforeCursor = enteredText.substr(0, $area.getSelection().start);
                            var wordList = textBeforeCursor.split(" ");
                            var lastWord = wordList[wordList.length - 2];
                            $.get("/SetupApi/SearchAutoText?term=" + lastWord + "&searchType=" + $area.options.searchType + "&displayLanguage=" + $area.options.displayLanguage, function (data) {
                                if (data != "") {
                                    $area.setSelection(textBeforeCursor.length - lastWord.length - 1, textBeforeCursor.length);
                                    $area.replaceSelection("");
                                    $area.insertAtCaretPos(data + " ");
                                }
                            });
                        }
                    }
